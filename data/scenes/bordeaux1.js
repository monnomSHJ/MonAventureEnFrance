...
import { bordeauxTransportData } from "../transportData.js";
import { getBordeaux1aScene } from "./bordeaux1a.js"; // 기차 선택 시
import { getBordeaux1bScene } from "./bordeaux1b.js"; // 버스 선택 시
import { getBordeaux1cScene } from "./bordeaux1c.js"; // 비행기 선택 시

export function getBordeaux1Scene() {
        ...
        contentHTML: `
            <div class="transport-content-wrap">
                <div class="transport-card-container">
                    ${bordeauxTransportData.map(transport => `
...
function setupReservationUI() {
    ...
    document.querySelectorAll(".transport-card-select").forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            const transport = bordeauxTransportData.find(h => h.id === id);
    ...
    confirmBtn.addEventListener("click", () => {
        ...
        const transport = bordeauxTransportData.find(h => h.id === selectedTransportID);
        ...
        btn2.onclick = () => {
            ...
            btn1.onclick = () => {
                ...
                if (selectedTransportID == "train") {
                    loadScene(getBordeaux1aScene());
                } else if (selectedTransportID == "bus") {
                    loadScene(getBordeaux1bScene());
                } else if (selectedTransportID == "airport") {
                    loadScene(getBordeaux1cScene());
                }
            }
